...
public class VehicleRepositoryImpl implements IVehicleRepository {
    ...
    public VehicleRepositoryImpl() { this("vehicles.csv"); }
    ...
    public Vehicle getVehicle(String id) {
        for (Vehicle v : vehicles)
            if (v.getId().equals(id)) return cloneVehicle(v);
        return null;
    }
    ...
    public List<Vehicle> getVehicles() {
        ...
        for (Vehicle v : vehicles) copy.add(cloneVehicle(v));
        return copy;
    }

    private Vehicle cloneVehicle(Vehicle v) {
        if (v instanceof Car c)
            return new Car(c.getId(), c.getBrand(), c.getModel(), c.getYear(), c.getPrice(), c.isRented());
        else if (v instanceof Motorcycle m)
            return new Motorcycle(m.getId(), m.getBrand(), m.getModel(), m.getYear(), m.getPrice(), m.isRented(), m.getCategory());
        return null;
    }

    private void save() {
        try (PrintWriter writer = new PrintWriter(new FileWriter(FILE_NAME))) {
            for (Vehicle v : vehicles) writer.println(v.toCSV());
        } catch (IOException e) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if ("CAR".equals(parts[0]))
                    ...
                    vehicles.add(new Motorcycle(parts[1], parts[2], parts[3],
                            Integer.parseInt(parts[4]), Double.parseDouble(parts[5]),
                            Boolean.parseBoolean(parts[6]), parts[7]));
            }
